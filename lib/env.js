...
var isValidUdsPath = function (p) {
	...
	if (!statSync(udsPath.dir).isDirectory()) {
		return false;
	}
	
	if (existsSync(p)) {
		console.log("Unix Domain Socket already exists and will be removed",
		  {socket: p});
		unlinkSync(p);
	}
    return true;
...
module.exports.create = function (config) {
    ...
    const Env = {
        ...
        httpUdsPath: isValidUdsPath(config.httpUdsPath)? config.httpUdsPath:'',
        httpSafeUdsPath: isValidUdsPath(config.httpSafeUdsPath)? config.httpSafeUdsPath:'',
        websocketUdsPath: isValidUdsPath(config.websocketUdsPath)? config.websocketUdsPath:'',
        websocketPath: config.externalWebsocketURL,
        udsAccessAllowAll: Boolean(config.udsAccessAllowAll),
        logIP: config.logIP,
