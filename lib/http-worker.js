...
EVENTS.MONITORING = function (data) {
    ...
    */
    Object.keys(data).forEach(pid => {
        ...
        let userSeconds = data[pid].cpu.user / 1000000;
        let systemSeconds = data[pid].cpu.system / 1000000;
        cpuUserMetric.set({pid: pid, type: data[pid].type}, userSeconds);
        cpuSystemMetric.set({pid: pid, type: data[pid].type}, systemSeconds);
        let sum = userSeconds + systemSeconds;
        let percent = sum / (Monitoring.interval/1000);
        cpuTotalMetric.set({pid: pid, type: data[pid].type}, sum);
        cpuPercentMetric.set({pid: pid, type: data[pid].type}, percent);
    });
