...
nThen(function (w) {
	const listenOptions = Env.httpUdsPath ? {
		path: Env.httpUdsPath,
		readableAll: true,
		writableAll: true
	} : {
		port: Env.httpPort,
	...
	const safeListenOptions = Env.httpSafeUdsPath ? {
		path: Env.httpSafeUdsPath,
		readableAll: true,
		writableAll: true
	} : (Env.httpSafePort ? {
		port: Env.httpSafePort,
		host: Env.httpAddress
	} : null) ;
	
	server.listen(listenOptions, w());

    if (safeListenOptions) {
        let safeServer = Http.createServer(app);
        safeServer.listen(safeListenOptions, w());
    }
